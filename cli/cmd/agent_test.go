...
// agentInitInTempProject generates instruction copies in an isolated project so
// tests do not depend on ignored context/ files in the repository checkout.
func agentInitInTempProject(t *testing.T) (string, string) {
	t.Helper()
...
// the agent must not know the viewer: both tracked templates and files generated
// by agent init must contain no `viewer`/`sdtviewer` token. The match is
...
func TestAgentBrowserTemplatesViewerFree(t *testing.T) {
	...
	files := []string{
		filepath.Join(repoRoot, "internal", "templates", "instructions", "browser.md.tmpl"),
		filepath.Join(repoRoot, "internal", "templates", "instructions", "browser-tools.md.tmpl"),
		filepath.Join(dir, "context", "instructions", "browser.md"),
		filepath.Join(dir, "context", "instructions", "browser-tools.md"),
	}
...
// the agent must not know the viewer: both vector module templates and files
// generated by agent init must contain no `viewer`/`sdtviewer` token. The match is
// word-boundary, because a plain search false-positives on "review"; the scope is
// this module only — the set-wide gate belongs to the portability-audit line.
func TestAgentVectorTemplatesViewerFree(t *testing.T) {
	...
	repoRoot, dir := agentInitInTempProject(t)
	files := []string{
		filepath.Join(repoRoot, "internal", "templates", "instructions", "vector.md.tmpl"),
		filepath.Join(repoRoot, "internal", "templates", "instructions", "vector-svg.md.tmpl"),
		filepath.Join(repoRoot, "internal", "templates", "instructions", "vector-tools.md.tmpl"),
		filepath.Join(dir, "context", "instructions", "vector.md"),
		filepath.Join(dir, "context", "instructions", "vector-svg.md"),
		filepath.Join(dir, "context", "instructions", "vector-tools.md"),
	}
	for _, path := range files {
		data, err := os.ReadFile(path)
		if err != nil {
