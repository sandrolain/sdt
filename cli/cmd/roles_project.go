...
func roleProjectLayer(r roleDescriptor, f roleProjectFacts) string {
	...
	b.WriteString("## Project layer (" + "`context/roles/" + r.Slug + "`" + ")\n\n")
	b.WriteString("Repo-derived facts for this role. Lines are tagged by source:\n")
	b.WriteString("`" + roleTagRepo + "` from repo evidence, `" + roleTagUser + "` answered by the user,\n")
	b.WriteString("`" + roleTagAssumption + "` not derivable — resolve before treating it as a rule.\n")
	b.WriteString("Regenerated by `sdt agent roles init --force`; do not hand-edit here.\n\n")

	b.WriteString("- Stack: " + tagged(f.Stack, roleTagRepo, roleTagAssumption) + "\n")
	b.WriteString("- Build: " + tagged(f.Build, roleTagRepo, roleTagAssumption) + "\n")
	b.WriteString("- Test: " + tagged(f.Test, roleTagRepo, roleTagAssumption) + "\n")
	b.WriteString("- Lint: " + tagged(f.Lint, roleTagRepo, roleTagAssumption) + "\n")

	if len(f.Layout) > 0 {
		b.WriteString("- Repo layout: " + "`" + strings.Join(f.Layout, "`, `") + "` " + roleTagRepo + "\n")
	}

	if owned := f.OwnedPaths[r.Slug]; len(owned) > 0 {
		b.WriteString("- Owned repo paths: " + "`" + strings.Join(owned, "`, `") + "` " + roleTagRepo + "\n")
	}

	if len(f.Conventions) > 0 {
		b.WriteString("- Conventions:\n")
		for _, c := range f.Conventions {
			b.WriteString("  - " + c + " " + roleTagRepo + "\n")
		}
	}

	for _, u := range f.UserAnswers {
		b.WriteString("- " + u + " " + roleTagUser + "\n")
	}

	b.WriteString("\nProject-specific conventions also live in `context/instructions/project.md`\n")
	b.WriteString("and the AGENTS.md project block — reference them, never restate them.\n")

	if len(f.Assumptions) > 0 {
		b.WriteString("\n### Open assumptions\n\n")
		b.WriteString("Not derivable from repo evidence and not answered by the user. Resolve with\n")
		b.WriteString("evidence or the ≤3-question questionnaire; never left as silent rules.\n\n")
		for _, a := range f.Assumptions {
			b.WriteString("- " + a + " " + roleTagAssumption + "\n")
		}
	}
	return b.String()
}
